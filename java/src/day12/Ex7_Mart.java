...
public class Ex7_Mart {
	...
	public static void main(String[] args) {
		...
		do {
			...
			case 3:
				...
				break;
			case 4:
				// < 내생각> 바구니의 물품이 30개 초과인지, 혹은 0개 이하인지 확인(그렇다면 진행할 필요 없음) 
				// 물건이 바구니 안에 적정수량 있다면 
				// 구매한 수량만큼 물건을 배열에서 삭제
				/* <강사님 + > 바구니에 담긴 목록 출력 - 최종 합계 출력 - 결제 금액을 입력 - 결제 진행 
				 * 금액이 부족하면 결제를 취소할 것인지 물어봄 - 취소시 장바구니 empty - 취소 안하면 장바구니 보관 
				 * 결재가 정상적으로 완료시 - 거스름돈 출력 - 바구니 비움 
				 */
	...
	/* 기능 : 스캐너를 이용하여 제품과 수량을 선택하면 제품 리스트에서 해당 제품을 수량에 맞게 꺼내서 제품을 돌려주는 메소드
	 * 매개변수 : scanner, 제품 list (마트가 관리하는) => scanner, product list[], int listCount
	 * 리턴타입 : 선택된 제품  Product
	 * 메소드명 :selectProduct
	 */
	...
	/* 기능 : 제품 리스트가 주어지면 해당 제품 리스트의 합계를 구하여 알려주는 메소
		...
		for (int i = 0; i < listCount; i++) {
			for (int j = 0; j < basketCount; j++) {
				...
				if (pi.getName().equals(pi.getName())) {
					//pi.amount는 재고량.. pj.amount는 구매하려고한 수량
